...
test('test', async ({ page }) => {
    ...
    await page.getByPlaceholder('ชื่อผู้ใช้').fill('009');
    ...
    await expect(page.locator("#spnCompanyName")).toHaveText('Baimiang Healthy Shop');  //VerifyText
    await page.waitForTimeout(3000);
    //});
    //เมนูผู้จัดจำหน่าย
    ...
    if (FindVendor) {
        ...
        if (text === expectedText) {
            ...
            await page.waitForTimeout(delay);
            await page.pause();
            await page.getByRole('tablist').filter({ hasText: '* อังกฤษ ไทย ลาว เวียดนาม' }).getByRole('tab', { name: 'ไทย' }).click();
            //await page.locator('#oliInforGeneralTapRsn > a').click();
            await page.waitForTimeout(delay);
